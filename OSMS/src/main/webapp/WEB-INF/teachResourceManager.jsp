		...
		<div class="row top4">
					<div class="col-md-2">
							    ...
							    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
							      <ul class="nav navbar-nav">
							         <li><a href="<%=path%>/showMyArea">教学信息管理</a></li>
									  <li class="active"><a href="<%=path%>/showTeachResourceManager">教学资源管理</a></li>
						...
						<ul class="nav nav-pills nav-stacked visible-lg visible-md">
						  ...
						  <li role="presentation" class="active"><a href="#">教学资源管理</a></li>
						  <li><a href="<%=path%>/showReplyStudent">回复学生问题</a></li>
						</ul>
					</div>
					<div class="col-md-10">
									  ...
									  <div class="modal-dialog" role="document">
									    <div class="modal-content">
									      <div class="modal-header">
									        ...
									        <h4 class="modal-title" id="myModalLabel">教学资源管理</h4>
									      </div>
										          	...
										          	<div class="col-md-12">
										          		 	<form enctype="multipart/form-data">
												                <div class="form-group">
												                    <input id="file-1" type="file"  class="file" name="file" data-overwrite-initial="false" data-min-file-count="1">
												                     <input type="hidden" id="filePath" name=""/>
												                     <input type="hidden" id="fileName" name=""/>   
												                      <p class="help-block">支持任何文件格式，大小不超过500M</p>  
												                </div>
												            </form>
										          	</div>
										          </div>
...
<script type="text/javascript">
		$("#file-1").fileinput({
		    uploadUrl: '<%=path%>/resourceController/upLoad', // you must set a valid URL here else you will get an error
		...
		$(function(){
			 $("#deleteBtn").click(function(){
				 var selectArray = $('#resourceTable').bootstrapTable('getSelections');
				 if(selectArray.length < 1){
					 showMsg("操作错误","请选择删除行");
				 }else{
					 var ids ="";
					 for(var i = 0;i < selectArray.length;i++){
						 ids += selectArray[i].resourceId+",";
					 }
					 ids = ids.substring(0,ids.length-1);
					 $.post("<%=path%>/resourceController/deleteResource",{"ids":ids},function(data){
							if(data.resultType == "1"){
								var opt = {
										url: "<%=path%>/resourceController/getResourceData",
								        silent: true,
								    };
							    $('#resourceTable').bootstrapTable('refresh',opt);
								showMsg("提示","删除成功");
							}else{
			...
			$("#addSubmit").click(function(){
				...
				$.post('<%=path%>/resourceController/add',{'resourceName':fileName,'resourceUrl':filePath},function(data){
					if(data.resultType == "1"){
						var opt = {
						        url: "<%=path%>/resourceController/getResourceData",
						        silent: true,
